...
void loop() {
    ...
    if (digitalRead(IO_0) == 0 && digitalRead(IO_1) == 0) {       // Both sensros are on the white, go forward.
      EmoroServo.write(SERVO_1, Lfwd);                            // left servo forward 
      EmoroServo.write(SERVO_0, Rfwd);                            // right servo forward
    }
      ...
      follow=1;                                                   // Goal reached - exit following mode
    }
